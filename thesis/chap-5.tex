...
Table \ref{table:eval_failed} shows other experiments that yielded lower results.

TF-IDF + LR achieved slightly worse performance than BoW + LR (Table \ref{table:eval}) in F1 score. Although the F1 scores of 'Problematic' and 'Questionable' classes suffered greatly. A hypothesis on the low TF-IDF score might be attributed to word-level noises that could disrupt these words from being included in the TF-IDF vocabulary, despite their potential importance.

GloVE, specifically the glove-6B-300d achieved worse performance than both BoW and TF-IDF.

Hi-BERT-LSTM model is a hierarchical model with BERT encoder, but followed by two layers of Bi-LSTM layers and a max-pooling layer instead of MLP. Inspired by \cite{khandve-2022-hierarchical-longdoc}

% Hi-MAGPIE with metadata is similar to Hi-MAGPIE model, but the outlet metadata is inputted into a single layer MLP, which then are concatenated with the sequence representation following the transformer encoder layers.
