...
% \subsection{Hierarchical Transformer with Metadata}
...
%         \tikzstyle{chunk} = [rectangle, draw, fill=gray!20]
%         \tikzstyle{label} = [text centered, font=\footnotesize]
%         \tikzstyle{input} = [rectangle, draw, fill=brown!20]

%         \node[input] (text_input) [draw] at (-5, 0) {Text Input};
%         \node[input] (meta_input) [draw] at (5, 0) {Metadata Input};

%         \node[chunk] (chunk1) [draw] at (-5, -1) {Chunk 1};
%         \node[chunk] (chunk2) [draw] at (-5, -1.5) {Chunk 2};
%         \node[chunk] (chunk3) [draw] at (-5, -2) {Chunk \(n\)};

%         \draw[arrow] (text_input) -- (chunk1);

%         \node (encoder) [draw, circle, fill=green!20] at (-5, -4) {LLM encoder};

%         \draw[arrow] (chunk3) -- (encoder);

%         \node[chunk] (embedding1) [draw] at (-5, -6) {Embeddings 1};
%         \node[chunk] (embedding2) [draw] at (-5, -6.5) {Embeddings 2};
%         \node[chunk] (embedding3) [draw] at (-5, -7) {Embeddings \(n\)};

%         \draw[arrow] (encoder) -- (embedding1);

%         \node (tf_layer1) [draw, rectangle, fill=green!20] at (-5, -8) {Transformer layer 1};
%         \node (tf_layer2) [draw, rectangle, fill=green!20] at (-5, -8.5) {Transformer layer 2};

%         \draw[arrow] (embedding3) -- (tf_layer1);

%         \node[chunk] (pooled1) [draw] at (-5, -9.5) {Pooled emb 1};
%         \node[chunk] (pooled2) [draw] at (-5, -10) {Pooled emb 2};
%         \node[chunk] (pooled3) [draw] at (-5, -11) {Pooled emb \(n\)};
        
%         \draw[arrow] (tf_layer2) -- (pooled1);
        
%         \node (mlp_m) [draw, circle, fill=green!20] at (5, -6) {MLP};
        
%         \node (mlp) [draw, circle, fill=green!20] at (-5, -12) {MLP};
        

%         \draw[arrow] (pooled3) -- (mlp);

%         \node (softmax) [draw, fill=cyan!20] at (-5, -13) {Softmax};

%         \draw[arrow] (mlp) -- (softmax);

%         \node (pred) [draw, fill=red!20] at (-5, -14) {Predicted class};

%         \draw[arrow] (softmax) -- (pred);
%     \end{tikzpicture}
%     \caption{Hierarchical transformer with metadata architecture}
%     \label{fig:hi_m_model}
% \end{figure}
