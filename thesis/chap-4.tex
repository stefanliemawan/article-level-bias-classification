...
\begin{figure}[htbp]
    ...
    \begin{tikzpicture}
        ...
        \node[chunk] (logits3) [draw] at (5, -6) {Logits 3};
    ...
    \end{tikzpicture}
    \caption{Chunk-based BERT fine-tuning. The input article is split into chunks, each chunk is processed by the model as a mini batch, and the resulting logits are pooled before applying softmax operation.}
...
\begin{figure}[htbp]
    ...
    \begin{tikzpicture}
        ...
        \node[label] at (4, -4.5) {extract last hidden state};
        ...
        \node[chunk] (embedding3) [draw] at (5, -6) {Embeddings 3};
        ...
        \node (dropout) [draw, rectangle, fill=blue!20] at (0, -10) {Dropout};
        \draw[arrow] (tf_layer2) -- (dropout);

        \node[label] at (4, -10.5) {extract CLS token};

        \node[chunk] (cls1) [draw] at (-5, -11.5) {CLS repr 1};
        \node[chunk] (cls2) [draw] at (0, -11.5) {CLS repr 2};
        \node[chunk] (cls3) [draw] at (5, -11.5) {CLS repr 3};

        \draw[arrow] (dropout) -- (cls1);
        \draw[arrow] (dropout) -- (cls2);
        \draw[arrow] (dropout) -- (cls3);

        \node (mlp) [draw, circle, fill=green!20] at (0, -13.25) {MLP};
        \node[label] at (4, -13) {two linear layers with ReLU and dropouts};

        \draw[arrow] (cls1) -- (mlp);
        \draw[arrow] (cls2) -- (mlp);
        \draw[arrow] (cls3) -- (mlp);

        \node (softmax) [draw, fill=cyan!20] at (0, -14.5) {Softmax};
